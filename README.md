...
|---|---|---|---|
| Compositor | `swayfx` | **built and working** (`./scripts/install-swayfx.sh`) | builds [WillPower3309/swayfx](https://github.com/WillPower3309/swayfx) 0.5.3 from source, pinned against SceneFX 0.4.1 and wlroots 0.19.0 as meson subprojects (wlroots' API isn't stable enough to rely on Fedora's system package). Installs to `/usr/local` so it never touches Fedora's dnf-managed `/usr/bin/sway` -- that stays as a guaranteed fallback (switch to a TTY and run it directly if SwayFX ever fails to start). Fedora's GCC/C23 toolchain is newer than wlroots 0.19.0 was written against, so the script builds with `CFLAGS=-Wno-error` to downgrade a couple of otherwise-harmless strict warnings back from hard errors. The script also registers `/usr/local/lib64` with the dynamic linker (needed for `libscenefx`/`libwlroots` to be found) and renames the installed session entry to `Name=SwayFX` so it's distinguishable from vanilla Sway at the login screen. The `blur`/`corner_radius`/`shadow*`/`default_dim_inactive` lines in `dotfiles/.config/sway/config` are now uncommented |
| Lock screen | `swaylock-effects` | **built and working** (`./scripts/install-swaylock-effects.sh`) | builds [ErikReider/swaylock-effects](https://github.com/ErikReider/swaylock-effects) from source to `/usr/local` (the COPR `pheeef/swaylock-effects` installs to the same path as Fedora's own `swaylock` package with no `Provides`/`Obsoletes`, risking a file conflict -- and a broken lock screen is a much worse failure mode than a broken compositor, so this keeps `/usr/bin/swaylock` untouched as a fallback instead). `dotfiles/.config/swaylock/config` already has the effects options (`screenshots`, `effect-blur`, `clock`, etc.) -- they were just silently ignored by vanilla swaylock until now |
...
| Touchpad gestures | `libinput-gestures` | not packaged | `./scripts/install-libinput-gestures.sh` -- installs from upstream ([bulletmark/libinput-gestures](https://github.com/bulletmark/libinput-gestures)) rather than COPR, since COPR chroots lag behind current Fedora releases (hit "chroot not found" on Fedora 44) and a pip/pipx install would put the binary in `~/.local/bin`, not matching the systemd service's hardcoded `ExecStart=/usr/bin/libinput-gestures`. Upstream's own installer places it at `/usr/bin` directly |
