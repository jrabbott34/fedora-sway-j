...
|---|---|---|---|
| Compositor | `swayfx` | not packaged | `./scripts/install-swayfx.sh` -- builds [WillPower3309/swayfx](https://github.com/WillPower3309/swayfx) 0.5.3 from source, pinned against SceneFX 0.4.1 and wlroots 0.19.0 as meson subprojects (wlroots' API isn't stable enough to rely on Fedora's system package). Installs to `/usr/local` so it never touches Fedora's dnf-managed `/usr/bin/sway` -- that stays as a guaranteed fallback. Until you've built and verified it, the `blur`/`corner_radius`/`shadow*`/`default_dim_inactive` lines in `dotfiles/.config/sway/config` stay commented out, since vanilla Sway doesn't understand them |
